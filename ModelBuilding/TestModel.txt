...
def load_datasets(dataset_dir):
    features = np.load(os.path.join(dataset_dir, "features_no_955_less.npy"), allow_pickle=True)
    targets = np.load(os.path.join(dataset_dir, "targets_no_955_less.npy"), allow_pickle=True)
    return features, targets


# Define paths
directory_path = "StockData/2022-01-01-2024-02-08/TrainingData/AAPL_MSFT"
model_path = "StockData/2022-01-01-2024-02-08/Models/AAPL_MSFT/1.88.hdf5"  # Update this path to your model's HDF5 file
...
model = buildMyNetwork(input_shape=val_features[0].shape)
...
#Define the acceptance range based on scale
lower_bound = -.8 * scale
upper_bound = .8 * scale

# Create masks to identify values within the acceptance range for both predictions and targets
mask_predictions = np.logical_and(predicted_means > lower_bound, predicted_means < upper_bound)
mask_targets = np.logical_and(val_targets > lower_bound, val_targets < upper_bound)

# Combine the masks to identify pairs where both predicted_means and val_targets are within the range
combined_mask = np.logical_and(mask_predictions, mask_targets)

# Invert the mask to filter out the pairs where both values are within the range
final_mask = np.logical_not(combined_mask)

# Apply the final mask to filter out the values for error calculation
filtered_absolute_errors = np.abs(predicted_means[final_mask] - val_targets[final_mask])

# Calculate performance metrics using the filtered values
mean_absolute_error = np.mean(filtered_absolute_errors)
percentile_50 = np.percentile(filtered_absolute_errors, 50)
percentile_75 = np.percentile(filtered_absolute_errors, 75)
percentile_90 = np.percentile(filtered_absolute_errors, 90)
percentile_95 = np.percentile(filtered_absolute_errors, 95)
percentile_99 = np.percentile(filtered_absolute_errors, 99)
...
print("Percent: 50: ", percentile_50)

# Confidence intervals and plotting code remains unchanged
...
for i in range(0, len(predicted_means), chunk_size):
    ...
    plt.scatter(range(i, end), (val_targets[final_mask][i:end] ), color='blue', label='Actual')
    plt.scatter(range(i, end), predicted_means[final_mask][i:end], color='red', label='Predicted', alpha=0.5)
