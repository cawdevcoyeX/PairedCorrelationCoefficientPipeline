...
def calculate_pearson_correlation(directory, output_root, window_length=25):
    ...
    for symbol1, symbol2 in combinations(symbols, 2):
        ...
        for file1 in symbol1_files:
            if 'StdDev' in file1 or 'Min' in file1:
                continue
            time_step1 = get_time_step(file1)
            for file2 in symbol2_files:
                if 'StdDev' in file2 or 'Min' in file1:
                    continue
                time_step2 = get_time_step(file2)
                if time_step1 == time_step2:
                    df1 = pd.read_csv(os.path.join(symbol1_dir, file1), usecols=['timestamp', 'EMA_2', 'volume','PctChange_2','trade_count','closing_bid_px','closing_ask_px','total_trades'])
                    df2 = pd.read_csv(os.path.join(symbol2_dir, file2), usecols=['timestamp', 'EMA_2', 'volume','PctChange_2','trade_count','closing_bid_px','closing_ask_px','total_trades'])
                    ...
                    merged_df = pd.merge(df1, df2, on='timestamp', how='inner').dropna()

                    #merged_df.dropna(inplace=True)

                    x = np.array(merged_df[symbol1 + '_EMA_2'], dtype=np.float64)
                    y = np.array(merged_df[symbol2 + '_EMA_2'], dtype=np.float64)
                    n = len(x)
                    ...
                    merged_df['slope_EMA_2'] = slope
                    merged_df['intercept_EMA_2'] = intercept
                    merged_df['pearson_correlation_EMA_2'] = correlation_coeffs
